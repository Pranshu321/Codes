...
//  For compatibility, implement std::invoke for C++11 and C++14
        ...
        struct Invoker<true, false>
        {
            ...
            inline static auto invoke(F T::*f, T1 &&t1, Args &&...args) -> decltype((InvokerHelper<std::is_base_of<T, typename std::decay<T1>::type>::value>::get(std::forward<T1>(t1)).*f)(std::forward<Args>(args)...))
            {
        ...
        {
            template <class T, class F, class T1, class... Args>
            inline static auto invoke(F T::*f, T1 &&t1, Args &&...args) -> decltype(InvokerHelper<std::is_base_of<T, typename std::decay<T1>::type>::value>::get(t1).*f)
            {
