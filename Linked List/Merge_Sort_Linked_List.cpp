...
{ // This is Function where we merge the two linked list.
    node *ans = NULL;
...
void Find_Middle(node *cur, node **first, node **second)
{ // this is the function in which we finding the middle element
    node *fast;
    node *slow;
    slow = cur;
    fast = cur->next;
...
{ // main function performing merge sort.
    node *cur = *head;
    node *first;
    node *second;
